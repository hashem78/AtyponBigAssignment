...
public class Mediator<Q extends Query, C extends Command> {
    ...
    <Q2 extends Q, R> Mediator<Q, C> register(Class<Q2> query, QueryHandler<Q2, R> handler) {
        queryMap.putIfAbsent(query, handler);
        return this;
    }

    <C2 extends C, R> Mediator<Q, C> register(Class<C2> command, CommandHandler<C2, R> handler) {
        commandMap.putIfAbsent(command, handler);
        return this;
    }

    @SuppressWarnings("unchecked")
    public <R> R run(Q query) {
        QueryHandler<Q, R> handler = (QueryHandler<Q, R>)queryMap.get(query.getClass());
        return handler.handle(query);
    }

    @SuppressWarnings("unchecked")
    public <R> R run(C command) {
